...
// Note: T must be a pointer type, or stuff will screw up. To make a pointer last longer
// than this object, assign nullptr to it and it won't free the old one.
template <typename T>
class autoptr {
    T ptr;
...
public:
    ...
    const T& operator[](int id) const { return getconst(id); }

    operator T*() { return ptr; }

    operator const T*() const { return ptr; }

    T& append(const T& item) { return (get(count) = item); }
