...
bool path_is_absolute(const char* path) {
    return ((isalpha(path[0]) && (':' == path[1]) &&
             (('\\' == path[2]) ||
              ('/' == path[2]))) /* drive letter + root, e.g. C:\dir or C:/dir */
            || ('\\' == path[0]) ||
            ('/' == path[0])); /* just root, e.g. \dir or /dir */
}

char get_native_path_separator() { return '\\'; }
...
FileHandleType open_file(const char* path, FileOpenMode mode, FileOpenError* error) {
    ...
    out_handle = CreateFileW(u16_str.c_str(), access_mode, share_mode, NULL,
