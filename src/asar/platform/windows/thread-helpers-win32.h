...
struct function_pointer_wrapper /*have this struct at global level*/
{
    ...
    static void __stdcall execute_fiber(void* parameter) {
        return fiber_callback(parameter);
    }

    static unsigned long (*thread_callback)(void*);
    static unsigned long __stdcall execute_thread(void* parameter) {
        return thread_callback(parameter);
    }
};
...
bool run_as_fiber(functor&& callback) {
    ...
    auto fiber = CreateFiberEx(16 * 1024 * 1024, 16 * 1024 * 1024, 0,
                               &function_pointer_wrapper::execute_fiber, &wrapper);
...
bool run_as_thread(functor&& callback) {
    ...
    auto thread =
            CreateThread(NULL, 16 * 1024 * 1024,
                         &function_pointer_wrapper::execute_thread, &wrapper, 0, NULL);
