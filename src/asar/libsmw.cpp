...
const unsigned char* romdata =
        nullptr;  // NOTE: Changed into const to prevent direct write access - use
                  // writeromdata() functions below
int romlen;
static bool header;
...
static void addromwriteforbank(int snesoffset, int numbytes) {
    ...
    if (insertpos > 0 &&
        (writtenblocks[insertpos - 1].snesoffset & 0xFF0000) == currentbank &&
        writtenblocks[insertpos - 1].snesoffset +
                        writtenblocks[insertpos - 1].numbytes >=
                snesoffset) {
        // Merge if we overlap with a preceding block
        int firstend = writtenblocks[insertpos - 1].snesoffset +
                       writtenblocks[insertpos - 1].numbytes;
    ...
    while (insertpos < writtenblocks.count &&
           (writtenblocks[insertpos].snesoffset & 0xFF0000) == currentbank &&
        ...
        int secondend =
                writtenblocks[insertpos].snesoffset + writtenblocks[insertpos].numbytes;
...
static void addromwrite(int pcoffset, int numbytes) {
    ...
    // Basically, the purpose of this is to sort all ROM writes into banks for the sake
    // of cleanness
...
int ratsstart(int snesaddr) {
    ...
    for (int i = 0x10000; i >= 0; i--) {
        if (!strncmp((const char*)start + i, "STAR", 4) &&
            (start[i + 4] ^ start[i + 6]) == 0xFF &&
            (start[i + 5] ^ start[i + 7]) == 0xFF) {
            if ((start[i + 4] | (start[i + 5] << 8)) > 0x10000 - i - 8 - 1)
                return pctosnes((int)(start - romdata + i));
            return -1;
        }
    }
    return -1;
...
static void handleprot(int loc, char* name, int len, const unsigned char* contents) {
    ...
    if (!strncmp(name, "PROT", 4)) {
        ...
        len /= 3;
        for (int i = 0; i < len; i++)
            removerats((contents[(i * 3) + 0]) | (contents[(i * 3) + 1] << 8) |
                               (contents[(i * 3) + 2] << 16),
                       0x00);
    }
}
...
static inline int trypcfreespace(int start, int end, int size, int banksize,
                                 int minalign, unsigned char freespacebyte) {
    while (start + size <= end) {
        if (((start + 8) & ~banksize) !=
                    ((start + size - 1) & ~banksize &
                     0xFFFFFF)  // if the contents won't fit in this bank...
            && (start & banksize & 0xFFFFF8) !=
                       (banksize &
                        0xFFFFF8)  // and the RATS tag can't fit in the bank either...
        ) {
            start &= ~banksize & 0xFFFFFF;  // round it down to the start of the bank,
            start |= banksize &
                     0xFFFFF8;  // then round it up to the end minus the RATS tag...
            continue;
        }
        if (minalign) {
        ...
        if (!strncmp((const char*)romdata + start, "STAR", 4) &&
            (romdata[start + 4] ^ romdata[start + 6]) == 0xFF &&
            (romdata[start + 5] ^ romdata[start + 7]) == 0xFF) {
        ...
        for (int i = 0; i < size; i++) {
            if (romdata[start + i] != freespacebyte) {
                ...
                start += i;
                if (!i)
                    start++;  // this could check for a rats tag instead, but somehow I
                              // think this will give better performance.
                bad = true;
                break;
            }
        }
...
// This function finds a block of freespace. -1 means "no freespace found", anything
// else is a PC address. isforcode=false tells it to favor banks 40+, true tells it to
// avoid them entirely. It automatically adds a RATS tag.

int getpcfreespace(int size, bool isforcode, bool autoexpand, bool respectbankborders,
                   bool align, unsigned char freespacebyte) {
    if (!size)
        return 0x1234;  // in case someone protects zero bytes for some dumb reason.
    // You can write zero bytes to anywhere, so I'll just return something that
    // removerats will ignore.
    ...
    rebootlorom:
        if (romlen > 0x200000 && !isforcode) {
            int pos = trypcfreespace(0x200000 - 8,
                                     (romlen < 0x400000) ? romlen : 0x400000, size,
                                     respectbankborders ? 0x7FFF : 0xFFFFFF,
                                     align                                  ? 0x7FFF
                                     : (respectbankborders || size < 32768) ? 0
                                                                            : 0x7FFF,
                                     freespacebyte);
            if (pos >= 0) return pos;
        }
    ...
    if (mapper == hirom) {
        if (isforcode) {
            for (int i = 0x8000; i < min(romlen, 0x400000); i += 0xFFFF) {
                int space = trypcfreespace(i, min(i + 0x7FFF, romlen), size, 0x7FFF,
                                           align ? 0xFFFF : 0, freespacebyte);
    ...
    if (mapper == exlorom) {
        // RPG Hacker: Not really 100% sure what to do here, but I suppose this
        // simplified code will do and we won't need all the complicated stuff from
        // LoROM above?
        if (isforcode) {
            trypcfreespace(0, min(romlen, 0x200000), size, 0x7FFF, align ? 0x7FFF : 0,
                           freespacebyte);
        }
    ...
    if (mapper == sfxrom) {
        ...
        return trypcfreespace(0x80000, romlen, size, 0x7FFF, align ? 0x7FFF : 0,
                              freespacebyte);
    }
    if (mapper == sa1rom) {
    rebootsa1rom:
        ...
        for (int i = 0; i < 8; i++) {
            ...
            int pos = trypcfreespace(sa1banks[i] ? sa1banks[i] : 0x80000,
                                     sa1banks[i] + 0x100000, size, 0x7FFF,
                                     align ? 0x7FFF : 0, freespacebyte);
            if (pos >= 0) return pos;
        }
    ...
    if (mapper == bigsa1rom) {
        if (!isforcode && romlen > 0x400000) {
            int pos = trypcfreespace(0x400000, romlen, size, 0xFFFF, align ? 0xFFFF : 0,
                                     freespacebyte);
            if (pos >= 0) return pos;
        }
        int pos = trypcfreespace(0x080000, romlen, size, 0x7FFF, align ? 0x7FFF : 0,
                                 freespacebyte);
        if (pos >= 0) return pos;
    }
    return -1;
...
void WalkMetadata(int loc, void (*func)(int loc, char* name, int len,
                                        const unsigned char* contents)) {
    int pcoff = snestopc(loc);
    if (strncmp((const char*)romdata + pcoff - 8, "STAR", 4)) return;
    const unsigned char* metadata = romdata + pcoff;
    while (is_upper(metadata[0]) && is_upper(metadata[1]) && is_upper(metadata[2]) &&
           is_upper(metadata[3])) {
        if (!strncmp((const char*)metadata, "STOP", 4)) {
            metadata = romdata + pcoff;
            while (is_upper(metadata[0]) && is_upper(metadata[1]) &&
                   is_upper(metadata[2]) && is_upper(metadata[3])) {
                if (!strncmp((const char*)metadata, "STOP", 4)) {
                    break;
                }
                func(pctosnes((int)(metadata - romdata)),
                     (char*)const_cast<unsigned char*>(metadata), metadata[4],
                     metadata + 5);
...
bool openrom(const char* filename, bool confirm) {
    ...
    memset(const_cast<unsigned char*>(romdata) + romlen, 0x00,
           (size_t)(16 * 1024 * 1024 - romlen));
    if (confirm && snestopc(0x00FFC0) + 21 < (int)romlen &&
        strncmp((const char*)romdata + snestopc(0x00FFC0), "SUPER MARIOWORLD     ",
                21)) {
        closerom(false);
        openromerror = header ? error_id_open_rom_not_smw_extension
...
uint32_t closerom(bool save) {
    ...
    if (thisfile != InvalidFileHandle && save && romlen) {
        ...
        // do a quick re-read of the header, and include that in the crc32 calculation
        // if necessary
        {
            uint8_t* filedata =
                    (uint8_t*)malloc(sizeof(uint8_t) * (romlen + header * 512));
            if (header) {
                set_file_pos(thisfile, 0u);
                read_file(thisfile, filedata, 512);
            }
            memcpy(filedata + (header * 512), romdata,
                   sizeof(uint8_t) * (size_t)romlen);
...
static unsigned int getchecksum() {
    ...
    if ((romlen & (romlen - 1)) == 0) {
        ...
        // assume romlen is the sum of 2 powers of 2 - i haven't seen any real rom that
        // isn't, and if you make such a rom, fixing its checksum is your problem.
...
void fixchecksum() {
    // randomdude999: clear out checksum bytes before recalculating checksum, this
    // should make it correct on roms that don't have a checksum yet
    ...
    writeromdata_byte(snestopc(0x00FFDD),
                      (unsigned char)(((checksum >> 8) & 255) ^ 255));
}
