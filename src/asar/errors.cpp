...
struct asar_error_mapping {
    ...
    asar_error_mapping(asar_error_id inerrid, const char* iname,
...
static asar_error_mapping asar_errors[] = {
        ...
        {ERR(buffer_too_small),
         "The given buffer is too small to contain the resulting ROM."},
        {ERR(params_null), "params passed to asar_patch_ex() is null."},
        {ERR(params_invalid_size),
         "Size of params passed to asar_patch_ex() is invalid."},
        {ERR(cmdl_define_invalid), "Invalid define name in %s: '%s'."},
        {ERR(cmdl_define_override),
         "%s '%s' overrides a previous define. Did you specify the same define twice?"},
        {ERR(create_rom_failed), "Couldn't create ROM."},
        {ERR(open_rom_failed), "Couldn't open ROM."},
        {ERR(open_rom_not_smw_extension),
         "Doesn't look like an SMW ROM. (Maybe its extension is wrong?)"},
        {ERR(open_rom_not_smw_header),
         "Doesn't look like an SMW ROM. (Maybe it's headered?)"},
        {ERR(stddefines_no_identifier),
         "stddefines.txt contains a line with a value, but no identifier."},
        {ERR(stddefine_after_closing_quote),
         "Broken std defines. (Something after closing quote)"},
        ...
        {ERR(xkas_asar_conflict),
         "Using @xkas and @asar in the same patch is not supported."},
        {ERR(invalid_version_number), "Invalid version number."},
        {ERR(asar_too_old), "This version of Asar is too old for this patch."},

        {ERR(relative_branch_out_of_bounds),
         "Relative branch out of bounds. (Distance is %s)."},
        ...
        {ERR(superfx_invalid_short_address),
         "Invalid short address parameter: $%s. (Must be even number and $0000-$01FE)"},
        {ERR(superfx_invalid_register),
         "Invalid register for opcode; valid range is %d-%d."},
        ...
        {ERR(label_cross_assignment),
         "Setting labels to other non-static labels is not valid."},
        {ERR(macro_label_outside_of_macro), "Macro label outside of a macro."},
        {ERR(label_on_third_pass),
         "Internal error: A label was created on the third pass. Please create an "
         "issue on the official GitHub repository and attach a patch which reproduces "
         "the error."},
        {ERR(label_moving),
         "Internal error: A label is moving around. Please create an issue on the "
         "official GitHub repository and attach a patch which reproduces the error."},
        ...
        {ERR(invalid_label_missing_closer),
         "Invalid label name, missing array closer."},
        {ERR(multiple_subscript_operators), "Multiple subscript operators is invalid."},
        {ERR(invalid_subscript),
         "Invalid array subscript after first scope resolution."},
        ...
        {ERR(endstruct_without_struct),
         "endstruct can only be used in combination with struct."},
        ...
        {ERR(overriding_builtin_define),
         "Trying to set define '%s', which is the name of a built-in define and thus "
         "can't be modified."},
        {ERR(define_label_math),
         "!Define #= Label is not allowed with non-static labels."},
        ...
        {ERR(nested_macro_definition),
         "Nested macro definition: Trying to define a macro inside '%s', which is not "
         "supported."},
        ...
        {ERR(command_in_non_root_file),
         "This command may only be used in the root file."},
        ...
        {ERR(no_banks_with_ram_mirrors),
         "No banks contain the RAM mirrors in hirom or exhirom."},
        {ERR(no_freespace_norom), "Can't find freespace in norom."},
        {ERR(static_freespace_autoclean),
         "A static freespace must be targeted by at least one autoclean."},
        {ERR(static_freespace_growing), "A static freespace may not grow."},
        {ERR(no_freespace_in_mapped_banks),
         "No freespace found in the mapped banks. (Requested size: %s)"},
        {ERR(no_freespace), "No freespace found. (Requested size: %s)"},
        {ERR(freespace_limit_reached),
         "A patch may not contain more than %d freespaces."},

        {ERR(prot_not_at_freespace_start),
         "PROT must be used at the start of a freespace block."},
        {ERR(prot_too_many_entries), "Too many entries to PROT."},

        {ERR(autoclean_in_freespace), "autoclean used in freespace."},
        {ERR(autoclean_label_at_freespace_end),
         "Don't autoclean a label at the end of a freespace block, you'll remove some "
         "stuff you're not supposed to remove."},
        {ERR(broken_autoclean), "Broken autoclean command."},

        {ERR(pulltable_without_table),
         "Using pulltable when there is no table on the stack yet."},
        ...
        {ERR(org_label_forward),
         "org Label is only valid for labels earlier in the patch."},

        {ERR(skip_label_invalid), "skip Label is not valid."},

        {ERR(spc700_inline_no_base),
         "base is not implemented for architecture spc700-inline."},
        ...
        {ERR(warnpc_failed),
         "warnpc failed: Current position (%s) is after end position (%s)."},
        {ERR(warnpc_failed_equal),
         "warnpc failed: Current position (%s) is equal to end position (%s)."},
        ...
        {ERR(pushwarnings_without_pullwarnings),
         "warnings push without matching warnings pull."},
        {ERR(pullwarnings_without_pushwarnings),
         "warnings pull without matching warnings push."},

        {ERR(failed_to_open_file_access_denied),
         "Failed to open file '%s'. Access denied."},
        {ERR(failed_to_open_not_regular_file),
         "Failed to open file '%s'. Not a regular file (did you try to use a "
         "directory?)"},

        {ERR(bad_dp_base),
         "The dp base should be page aligned (i.e. a multiple of 256)"},
        {ERR(bad_dp_optimize),
         "Bad dp optimize value %s, expected: [none, ram, always]"},
        {ERR(bad_address_optimize),
         "Bad dp optimize value %s, expected: [default, ram, mirrors]"},
        ...
        {ERR(duplicate_param_name),
         "Duplicated parameter name: %s in creation of function %s"},
        ...
        {ERR(vararg_out_of_bounds),
         "Variadic macro parameter requested is out of bounds."},
        {ERR(vararg_must_be_last),
         "Variadic macro parameter must be the last parameter."},
        {ERR(invalid_global_label),
         "Global label definition contains an invalid label [%s]."},

        {ERR(spc700_addr_out_of_range),
         "Address %s out of range for instruction, valid range is 0000-1FFF"},
        {ERR(label_ambiguous),
         "Label (%s) location is ambiguous due to straddling optimization border."},

        {ERR(feature_unavaliable_in_spcblock),
         "This feature may not be used while an spcblock is active"},
        {ERR(endspcblock_without_spcblock),
         "Use of endspcblock without matching spcblock"},
        ...
        {ERR(spcblock_inside_struct),
         "Can not start an spcblock while a struct is still open"},
        ...
        {ERR(custom_spcblock_missing_macro),
         "Custom spcblock types must refer to a valid macro"},
        {ERR(spcblock_macro_doesnt_exist),
         "Macro specified to custom spcblock was not found"},
        {ERR(extra_spcblock_arg_for_type),
         "Only custom spcblock type takes a fourth argument"},
        {ERR(spcblock_macro_must_be_varadic),
         "Custom spcblock macros must be variadic"},
        {ERR(spcblock_macro_invalid_static_args),
         "Custom spcblock macros must have three static arguments"},
        {ERR(spcblock_custom_types_incomplete),
         "Custom spcblock types are not yet supported. One day."},
        {ERR(startpos_without_spcblock),
         "The startpos command is only valid in spcblocks"},
        ...
        {ERR(label_forward),
         "The use of forward labels is not allowed in this context."},
        {ERR(undefined_char), "'%s' is not defined in the character table"},

        {ERR(invalid_utf8),
         "Invalid text encoding detected. Asar expects UTF-8-encoded text. Please "
         "re-save this file in a text editor of choice using UTF-8 encoding."},
        {ERR(cmdl_utf16_to_utf8_failed),
         "UTF-16 to UTF-8 string conversion failed: %s."},

        {ERR(broken_command), "Broken %s command. %s"},
        {ERR(phantom_error),
         "A phantom error occurred. This is an Asar bug, please report it: "
         "https://github.com/RPGHacker/asar/issues"},
        ...
        {ERR(invalid_vararg),
         "Trying to use variadic macro parameter syntax to resolve a non variadic "
         "argument."},

        {ERR(macro_param_outside_macro),
         "Reference to macro parameter outside of macro"},
        {ERR(invalid_depth_resolve),
         "Invalid %s resolution depth: Trying to backwards-resolve a %s using %i '^', "
         "but current scope only supports up to %i '^'."},

        {ERR(platform_paths),
         "Platform-specific paths aren'supported. Please use platform-independent "
...
static_assert(sizeof(asar_errors) / sizeof(asar_errors[0]) == error_id_count,
              "asar_errors and asar_error_id are not in sync");
