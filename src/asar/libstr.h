...
class string {
public:
    const char* data() const { return cached_data; }
    ...
    char* raw() const { return cached_data; }

    int length() const { return is_inlined() ? inlined.len : allocated.len; }
    ...
    void truncate(int newlen) { resize(newlen); }
    ...
    void assign(const string& newstr) { assign(newstr, newstr.length()); }
    ...
    bool operator==(const char* right) const { return !strcmp(data(), right); }

    bool operator==(const string& right) const { return !strcmp(data(), right.data()); }

    bool operator!=(const char* right) const { return (strcmp(data(), right) != 0); }
    ...
    string(const char* newstr) : string() { assign(newstr); }
    string(const char* newstr, int newlen) : string() { assign(newstr, newlen); }
    string(const string& old) : string() { assign(old.data()); }

    string(string&& move) : string() { *this = move; }
    ...
    // Symbol files are currently the only thing that use text mode, anyways, and I
    // don't even know if the emulators that read them care about line endings.
    string& convert_line_endings_to_native() {
#if defined(windows)
        // RPG Hacker: This is quite stinky, but doing the replacement directly will
        // lead to a dead-lock. \x08 = backspace should never appear inside a string, so
        // I'm abusing it here.
        return qreplace("\n", "\x08").qreplace("\x08", "\r\n");
#else
        return *this;
...
private:
    ...
    static const int max_inline_length_ =
            ((sizeof(char*) + sizeof(int) * 2) * scale_factor) - 2;
    char* cached_data;
    int next_resize;
    struct si {
    ...
    void resize(int new_length) {
        ...
        if (new_length >= next_resize ||
            (!is_inlined() && new_length <= max_inline_length_)) {
            if (new_length > max_inline_length_ &&
                (is_inlined() ||
                 allocated.bufferlen <= new_length)) {  // SSO or big to big
                int new_size = bitround(new_length + 1);
                if (old_data == inlined.str) {
                    allocated.str = copy(old_data, min(length(), new_length),
                                         (char*)malloc(new_size));
                } else {
                    allocated.str = (char*)realloc(allocated.str, new_size);
                    old_data = inlined.str;  // this will prevent freeing a dead realloc
                                             // ptr
                }
                allocated.bufferlen = new_size;
                cached_data = allocated.str;
                next_resize = allocated.bufferlen;
            } else if (length() > max_inline_length_ &&
                       new_length <= max_inline_length_) {  // big to SSO
    ...
    bool is_inlined() const { return inlined.len != (unsigned char)-1; }
};
#define STR (string)

char* readfile(const char* fname, const char* basepath);
char* readfilenative(const char* fname);
bool readfile(const char* fname, const char* basepath, char** data,
              int* len);  // if you want an uchar*, cast it
...
inline string ftostr(double value) {
    // randomdude999: With 100 digits of precision, the buffer needs to be approx.
    // 311+100, but let's be safe here https://stackoverflow.com/questions/7235456
    char rval[512];
...
inline char* strqchr(const char* str, char key) {
    while (*str != '\0') {
        if (*str == key) {
            ...
            // Special case hack for ''', which is currently our official way of
            // handling the ' character. Even though it really stinks.
            if (str[0] == '\'' && str[1] == '\'' && str[2] == '\'') {
                ...
                do {
                    ...
                    // If we want to support backslash escapes, we'll have to add that
                    // right here.
                } while (*str != delimiter && *str != '\0');

                // This feels like a superfluous check, but I can't really find a clean
                // way to avoid it.
                if (*str == '\0') {
                    return nullptr;
                }
...
inline string substr(const char* str, int len) { return string(str, len); }
