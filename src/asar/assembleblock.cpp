...
enum spcblock_type { spcblock_nspc, spcblock_custom };
...
int snestopc_pick(int addr) { return snestopc(addr); }
...
inline void write1_65816(unsigned int num) {
    ...
    if (pass == 2) {
        ...
        if (pcpos < 0) {
            ...
            asar_throw_error(2, error_type_block,
                             error_id_snes_address_doesnt_map_to_rom,
                             hex((unsigned int)realsnespos, 6).data());
        }
...
const char* safedequote(char* str) {
    ...
    if (!tmp)
        asar_throw_error(0, error_type_block, error_id_garbage_near_quoted_string);
    return tmp;
}
...
int read2(int insnespos) {
    ...
    return romdata_r[addr] | (romdata_r[addr + 1] << 8);
}

int read3(int insnespos) {
    int addr = snestopc(insnespos);
    if (addr < 0 || addr + 3 > romlen_r) return -1;
    return romdata_r[addr] | (romdata_r[addr + 1] << 8) | (romdata_r[addr + 2] << 16);
}
...
// randomdude999: ns is still the string to prefix to all labels, it's calculated
// whenever namespace_list is changed
string ns;
string ns_backup;
...
string posneglabelname(const char** input, bool define) {
    ...
    if (label[0] == '-' || label[0] == '+') {
        ...
        if (!ismacro) {
            ...
            if (macrorecursion == 0 || macroposlabels == nullptr ||
                macroneglabels == nullptr) {
                if (!macrorecursion)
                    asar_throw_error(0, error_type_block,
                                     error_id_macro_label_outside_of_macro);
            } else {
                if (first == '+') {
                    *input = label;
                    output = STR ":macro_" + dec(calledmacros) + "_pos_" + dec(depth) +
                             "_" + dec((*macroposlabels)[depth]);
                    if (define) (*macroposlabels)[depth]++;
                } else {
                    *input = label;
                    if (define) (*macroneglabels)[depth]++;
                    output = STR ":macro_" + dec(calledmacros) + "_neg_" + dec(depth) +
                             "_" + dec((*macroneglabels)[depth]);
                }
            }
        }
    }
...
#define deref_rawname (*rawname)
    ...
    if (is_digit(*deref_rawname))
        asar_throw_error(1, error_type_block, error_id_invalid_label_name);
    if (*deref_rawname == ':') {
        deref_rawname++;
        name = ":";
    } else if (!in_struct && !in_sub_struct) {
        for (i = 0; (*deref_rawname == '.'); i++) deref_rawname++;
        if (!is_ualnum(*deref_rawname))
            asar_throw_error(1, error_type_block, error_id_invalid_label_name);
        if (i) {
    ...
    if (ismacro && !issublabel) {
        // RPG Hacker: Don't add the prefix for sublabels, because they already inherit
        // it from their parents' names.
        if (!macrorecursion || macrosublabels == nullptr)
            asar_throw_error(1, error_type_block,
                             error_id_macro_label_outside_of_macro);
    ...
    if (in_struct || in_sub_struct) {
        ...
        if (*deref_rawname != '.')
            asar_throw_error(1, error_type_block,
                             error_id_invalid_label_name);  // probably should be a
                                                            // better error. TODO!!!
        deref_rawname++;
    }
...
inline bool labelvalcore(const char** rawname, snes_label* rval, bool define,
                         bool shouldthrow) {
    string name = labelname(rawname, define);
    if (ns && labels.exists(ns + name)) {
        *rval = labels.find(ns + name);
    } else if (labels.exists(name)) {
        *rval = labels.find(name);
    } else {
        if (shouldthrow && pass) {
...
static void setlabel(string name, int loc = -1, bool is_static = false) {
    ...
    if (pass == 0) {
        if (labels.exists(name)) {
            movinglabelspossible = true;
            asar_throw_error(0, error_type_block, error_id_label_redefined,
                             name.data());
        }
        ...
        if (!labels.exists(name))
            asar_throw_error(2, error_type_block, error_id_label_on_third_pass);
        labelpos = labels.find(name).pos;
        if ((int)labelpos != loc && !movinglabelspossible) {
            if ((unsigned int)loc >> 16 != labelpos >> 16)
                asar_throw_error(2, error_type_block, error_id_label_ambiguous,
                                 name.raw());
            else if (labelpos == (dp_base + 0xFFu))
                asar_throw_error(2, error_type_block, error_id_label_ambiguous,
                                 name.raw());
            else if (errored)
                return;
            else
                asar_throw_error(2, error_type_block, error_id_label_moving);
        }
    }
}
...
static void cleartable() { thetable = table(); }
...
static int getfreespaceid() {
    ...
    if (freespaceidnext > max_num_freespaces)
        asar_throw_error(pass, error_type_fatal, error_id_freespace_limit_reached,
                         max_num_freespaces);
...
void checkbankcross() {
    ...
    } else if (((realsnespos ^ realstartpos) & mask) &&
               (((realsnespos - 1) ^ realstartpos) & mask)) {
...
void finishpass() {
    ...
    if (in_spcblock)
        asar_throw_error(0, error_type_block, error_id_missing_endspcblock);
    if (in_struct || in_sub_struct)
        asar_throw_error(pass, error_type_null, error_id_struct_without_endstruct);
    else if (pushpcnum && pass == 0)
        asar_throw_error(pass, error_type_null, error_id_pushpc_without_pullpc);
    else if (pushnsnum && pass == 0)
        asar_throw_error(pass, error_type_null, error_id_pushns_without_pullns);
    freespaceend();
...
static bool addlabel(const char* label, int pos = -1, bool global_label = false) {
    if (!label[0] || label[0] == ':')
        return false;  // colons are reserved for special labels
    ...
    if (posnegname.length() > 0) {
        ...
        if (*posneglabel != '\0' && *posneglabel != ':')
            asar_throw_error(0, error_type_block, error_id_broken_label_definition);
    ...
    if (label[strlen(label) - 1] == ':' || label[0] == '.' || label[0] == '?' ||
        ...
        if (global_label && (in_struct || in_sub_struct || label[0] == '?'))
            return false;
        ...
        bool requirecolon =
                (label[0] != '.' && label[1] != '.') && (in_struct || in_sub_struct);
        string name = labelname(&label, define);
        if (label[0] == ':')
            label++;
        else if (requirecolon)
            asar_throw_error(0, error_type_block, error_id_broken_label_definition);
        else if (global_label)
            return false;
        if (label[0])
            asar_throw_error(0, error_type_block, error_id_broken_label_definition);
...
string handle_print(char* input) {
    ...
    for (int i = 0; pars[i]; i++) {
        if (0)
            ...
            out += safedequote(pars[i]);
        else if (!stricmp(pars[i], "bytes"))
            out += dec(bytes);
        else if (!stricmp(pars[i], "freespaceuse"))
            out += dec(freespaceuse);
        else if (!stricmp(pars[i], "pc"))
            out += hex((unsigned int)(snespos & 0xFFFFFF), 6);
            ...
            verify_paren(params);
            if (numargs > 2)
                asar_throw_error(0, error_type_block, error_id_wrong_num_parameters);
            int precision = 0;
            ...
            *(arg1pos - 1) = '\0';  // allows more convenient comparsion functions
            if (!stricmp(pars[i], "bin")) {
                ...
                if (startidx == 64)
                    startidx--;  // always want to print at least one digit
                out += buffer + startidx;
            ...
            asar_throw_error(2, error_type_block, error_id_unknown_variable);
    }
    return out;
}
...
#define par word[1]
    ...
    while (numif == numtrue && word[0] && (!word[1] || strcmp(word[1], "=")) &&
           addlabel(word[0])) {
        word++;
        numwords--;
    }
    ...
    if (is("if") || is("elseif") || is("assert") || is("while")) {
        ...
        if (foundlabel && !foundlabel_static && !is("assert"))
            asar_throw_error(1, error_type_block, error_id_label_in_conditional,
                             word[0]);
        if (is("if") || is("while")) {
            if (0)
                ;
            ...
            if (!numif)
                asar_throw_error(1, error_type_block, error_id_misplaced_elseif);
            if (whilestatus[numif - 1].iswhile)
                asar_throw_error(1, error_type_block, error_id_elseif_in_while);
        ...
        // otherwise, must be assert command
            ...
            if (errmsg)
                asar_throw_error(2, error_type_block, error_id_assertion_failed,
                                 (string(": ") + errmsg).data());
            else
                asar_throw_error(2, error_type_block, error_id_assertion_failed, ".");
        }
    } else if (is0("endif") || is0("endwhile")) {
        if (!numif || (whilestatus[numif - 1].iswhile && is("endif")))
        ...
        if (whilestatus[numif - 1].iswhile)
            asar_throw_error(1, error_type_block, error_id_else_in_while_loop);
        ...
        if (pass == 2) {
            // RPG Hacker: This makes a pretty big assumption to calculate the size of
            // opcodes. However, this should currently only really be used for pseudo
            // opcodes, where it should always be good enough.
            if (recent_opcode_num > 0) {
                int opcode_size =
                        ((0xFFFFFF & realsnespos) - addrToLinePos) / recent_opcode_num;
                for (int i = 0; i < recent_opcode_num; ++i) {
                    extern AddressToLineMapping addressToLineMapping;
                    addressToLineMapping.includeMapping(
                            get_current_file_name(), get_current_line() + 1,
                            addrToLinePos + (i * opcode_size));
                }
            }
        }
        ...
        if (defines.exists(def)) {
            ...
            asar_throw_error(0, error_type_block, error_id_define_not_found,
                             def.data());
        }
    } else if (is0("error")) {
        ...
        // RPG Hacker: This used to be on pass 0, which had its merits (you don't want
        // to miss a potentially critical user-generated error, just because a bazillion
        // other errors are thrown in passes before it). However, I don't see how to
        // support print functions with this without moving it to pass 2. Suggestions
        // are welcome.
        asar_throw_error(2, error_type_block, error_id_error_command,
                         (string(": ") + out).data());
    } else if (is1("warn")) {
        string out = handle_print(par);
        ...
        if (stricmp(word[1], "push") == 0) {
            ...
            asar_throw_error(0, error_type_block, error_id_broken_command, "warnings",
                             "Unknown parameter");
        }
        ...
        if (stricmp(word[1], "enable") == 0) {
            ...
            if (warnid != warning_id_end) {
                ...
                asar_throw_error(0, error_type_block, error_id_invalid_warning_id,
                                 word[2], "warnings enable");
            }
        ...
        if (!addlabel(word[1], -1, true)) {
            asar_throw_error(1, error_type_block, error_id_invalid_global_label,
                             word[1]);
        }
    } else if (is2("check")) {
        if (stricmp(word[1], "title") == 0) {
            // RPG Hacker: Removed trimming for now - I think requiring an exact match
            // is probably better here (not sure, though, it's worth discussing)
            string expected_title = safedequote(word[2]);
            // randomdude999: this also removes leading spaces because itrim gets stuck
            // in an endless loop when multi is true and one argument is empty in hirom
            // the rom needs to be an entire bank for it to have a title, other modes
            // only need 0x8000 bytes
            if (romlen < ((mapper == hirom || mapper == exhirom)
                                  ? 0x10000
                                  : 0x8000))  // too short
            {
                if (!ignoretitleerrors)  // title errors shouldn't be ignored
                    asar_throw_error(0, error_type_block, error_id_rom_too_short,
                                     expected_title.data());
                else  // title errors should be ignored, throw a warning anyways
                    asar_throw_warning(0, warning_id_rom_too_short,
                                       expected_title.data());
            } else {
                string actual_title;
                ...
                if (strncmp(expected_title, actual_title, 21) != 0) {
                    if (!ignoretitleerrors)  // title errors shouldn't be ignored
                        asar_throw_error(
                                0, error_type_block, error_id_rom_title_incorrect,
                                expected_title.data(), actual_display_title.data());
                    else  // title errors should be ignored, throw a warning anyways
                        asar_throw_warning(0, warning_id_rom_title_incorrect,
                                           expected_title.data(),
                                           actual_display_title.data());
                }
            }
        ...
        if (!asarverallowed)
            asar_throw_error(0, error_type_block, error_id_start_of_file);
        if (!par) return;
        int dots = 0;
        int dig = 0;
        for (int i = 0; par[i]; i++) {
            if (par[i] == '.') {
                ...
                dig = 0;
                dots++;
            } else if (is_digit(par[i]))
                dig++;
            else
                asar_throw_error(0, error_type_block, error_id_invalid_version_number);
        }
        if (!dig || !dots || dots > 2)
            asar_throw_error(0, error_type_block, error_id_invalid_version_number);
        autoptr<char**> vers = split(par, '.');
        int vermaj = atoi(vers[0]);
        if (vermaj > asarver_maj)
            asar_throw_error(pass, error_type_fatal, error_id_asar_too_old);
        if (vermaj < asarver_maj) return;
        if (dots == 1) {
            if (strlen(vers[1]) != 2)
                asar_throw_error(0, error_type_block, error_id_invalid_version_number);
            // if (asarver_min<10 && asarver_bug<10 && strlen(vers[1])>2) error(0, "This
            // version of Asar is too old for this patch.");
            int verminbug = atoi(vers[1]);
            int tmpver = asarver_bug;
            if (tmpver > 9) tmpver = 9;
            if (asarver_min * 10 + tmpver < verminbug)
                asar_throw_error(pass, error_type_fatal, error_id_asar_too_old);
        } else {
            int vermin = atoi(vers[1]);
            if (vermin > asarver_min)
                asar_throw_error(pass, error_type_fatal, error_id_asar_too_old);
            int verbug = atoi(vers[2]);
            if (vermin == asarver_min && verbug > asarver_bug)
                asar_throw_error(pass, error_type_fatal, error_id_asar_too_old);
        }
        ...
        if (in_top_level_file()) {
            if (par)
                asar_throw_error(pass, error_type_fatal, error_id_cant_be_main_file,
                                 (string(" The main file is '") + par + "'.").data());
            else
                asar_throw_error(pass, error_type_fatal, error_id_cant_be_main_file,
                                 "");
        }
        ...
        if (foundlabel && !foundlabel_static)
            asar_throw_error(0, error_type_block, error_id_label_cross_assignment);
        ...
        if (!confirmname(newlabelname))
            asar_throw_error(0, error_type_block, error_id_invalid_label_name);
        ...
        if (forwardlabel)
            asar_throw_error(0, error_type_block, error_id_org_label_forward);
        if (num & ~0xFFFFFF)
            asar_throw_error(1, error_type_block, error_id_snes_address_out_of_bounds,
                             hex(num, 6).data());
        if ((mapper == lorom || mapper == exlorom) && (num & 0x408000) == 0x400000 &&
            (num & 0x700000) != 0x700000)
            asar_throw_warning(0, warning_id_set_middle_byte);
...
#define ret_error(errid)                              \
    {                                                 \
        asar_throw_error(0, error_type_block, errid); \
        return;                                       \
    }
#define ret_error_params(errid, ...)                               \
    {                                                              \
        asar_throw_error(0, error_type_block, errid, __VA_ARGS__); \
        return;                                                    \
    }
    else if (is("struct")) {
        // verifysnespos();
        ...
        if (structs.exists(word[1]) && pass == 0)
            ret_error_params(error_id_struct_redefined, word[1]);
...
#define ret_error_cleanup(errid)                      \
    {                                                 \
        in_struct = old_in_struct;                    \
        in_sub_struct = old_in_sub_struct;            \
        asar_throw_error(0, error_type_block, errid); \
        return;                                       \
    }
#define ret_error_params_cleanup(errid, ...)                       \
    {                                                              \
        in_struct = old_in_struct;                                 \
        in_sub_struct = old_in_sub_struct;                         \
        asar_throw_error(0, error_type_block, errid, __VA_ARGS__); \
        ...
        if (numwords == 3) {
            if (base & ~0xFFFFFF)
                ret_error_params_cleanup(error_id_snes_address_out_of_bounds,
                                         hex((unsigned int)base, 6).data());
            snespos = (int)base;
            startpos = (int)base;
        } else if (numwords == 4) {
            if (strcasecmp(word[2], "extends"))
                ret_error_cleanup(error_id_missing_extends);
            if (!confirmname(word[3]))
                ret_error_cleanup(error_id_struct_invalid_parent_name);
            string tmp_struct_parent = word[3];

            if (!structs.exists(tmp_struct_parent))
                ret_error_params_cleanup(error_id_struct_not_found,
                                         tmp_struct_parent.data());
...
#undef ret_error_params_cleanup
        ...
        if (numwords == 3 && strcasecmp(word[1], "align"))
            ret_error(error_id_expected_align);
        ...
        structure.struct_size =
                alignment * ((snespos - struct_base + alignment - 1) / alignment);
...
#undef ret_error
        ...
        // banned features when active: org, freespace(and variants), arch,
        // mapper,namespace,pushns
        if (arch != arch_spc700)
            asar_throw_error(0, error_type_block, error_id_spcblock_bad_arch);
        if (in_struct || in_sub_struct)
            asar_throw_error(0, error_type_block, error_id_spcblock_inside_struct);
        if (numwords < 2)
            asar_throw_error(0, error_type_block, error_id_spcblock_too_few_args);
        if (numwords > 4)
            asar_throw_error(0, error_type_block, error_id_spcblock_too_many_args);

        spcblock.destination = getnum(par);
        spcblock.type = spcblock_nspc;
        spcblock.macro_name = "";

        if (spcblock.destination & ~0xFFFF)
            asar_throw_error(0, error_type_block, error_id_snes_address_out_of_bounds,
                             hex(spcblock.destination, 6).data());

        if (numwords == 3) {
            if (!stricmp(word[2], "nspc"))
                spcblock.type = spcblock_nspc;
            else if (!stricmp(word[2], "custom"))
                asar_throw_error(0, error_type_block,
                                 error_id_custom_spcblock_missing_macro);
            else
                asar_throw_error(0, error_type_block, error_id_unknown_spcblock_type);
        } else if (numwords == 4) {
            if (!stricmp(word[2], "custom"))
                spcblock.type = spcblock_custom;
            else
                asar_throw_error(0, error_type_block,
        ...
        switch (spcblock.type) {
            ...
            case spcblock_custom:
                ...
                // mostly so we can leverage some cleanups we make in 2.0 for
                // practicality
                asar_throw_error(0, error_type_block,
                ...
                break;
            default:
                asar_throw_error(0, error_type_fatal, error_id_internal_error,
                                 "invalid spcblock type");
        }
        ...
        if (!in_spcblock)
            asar_throw_error(0, error_type_block, error_id_startpos_without_spcblock);
        ...
        unsigned int num = getnum(par);
        if (forwardlabel)
            asar_throw_error(0, error_type_block, error_id_base_label_invalid);
        if (num & ~0xFFFFFF)
            asar_throw_error(1, error_type_block, error_id_snes_address_out_of_bounds,
                             hex((unsigned int)num).data());
        snespos = (int)num;
        startpos = (int)num;
        optimizeforbank = -1;
        snespos_valid = realsnespos >= 0;
        ...
        if (forwardlabel)
            asar_throw_error(0, error_type_block, error_id_base_label_invalid);
        if (num & ~0xFF00)
            asar_throw_error(1, error_type_block, error_id_bad_dp_base,
                             hex((unsigned int)num, 6).data());
        dp_base = (int)num;
        ...
        if (!stricmp(par, "address")) {
            ...
            asar_throw_error(1, error_type_block, error_id_bad_address_optimize,
                             word[2]);
        }
        ...
        if (useram == -1)
            asar_throw_error(0, error_type_block, error_id_invalid_freespace_request);
        if (mapper == norom)
            asar_throw_error(0, error_type_block, error_id_no_freespace_norom);
        freespaceend();
        ...
        if (pass == 1) {
            ...
            if (fixedpos && freespaceorgpos[freespaceid])
                freespacepos[freespaceid] = snespos =
                        (freespaceid << 24) | freespaceorgpos[freespaceid];
            else
                freespacepos[freespaceid] = snespos =
                        (freespaceid << 24) |
                        getsnesfreespace(freespacelen[freespaceid], (useram != 0), true,
                                         true, align, freespacebyte[freespaceid]);
        }
        if (pass == 2) {
            if (fixedpos && freespaceorgpos[freespaceid] == -1)
                return;  // to kill some errors
            snespos = (freespaceid << 24) | freespacepos[freespaceid];
            resizerats(snespos & 0xFFFFFF, freespacelen[freespaceid]);
            if (freespaceleak[freespaceid] && leakwarn)
                asar_throw_warning(2, warning_id_freespace_leaked);
            if (fixedpos && freespaceorgpos[freespaceid] > 0 &&
                freespacelen[freespaceid] > freespaceorglen[freespaceid])
                asar_throw_error(2, error_type_block,
                                 error_id_static_freespace_growing);
            freespaceuse += 8 + freespacelen[freespaceid];
        }
        freespacestatic[freespaceid] = fixedpos;
        if (snespos < 0 && mapper == sa1rom)
            asar_throw_error(pass, error_type_fatal,
                             error_id_no_freespace_in_mapped_banks,
                             dec(freespacelen[freespaceid]).data());
        if (snespos < 0)
            asar_throw_error(pass, error_type_fatal, error_id_no_freespace,
                             dec(freespacelen[freespaceid]).data());
        bytes += 8;
        ...
        if (in_spcblock)
            asar_throw_error(0, error_type_block,
                             error_id_feature_unavaliable_in_spcblock);
        if (!ratsmetastate)
            asar_throw_error(2, error_type_block, error_id_prot_not_at_freespace_start);
        ...
        write1('T');
        if (num * 3 > 255)
            asar_throw_error(0, error_type_block, error_id_prot_too_many_entries);
        ...
        for (int i = 0; i < num; i++) {
            ...
            if (*labeltest)
                asar_throw_error(0, error_type_block, error_id_label_not_found,
                                 testlabel.data());
        ...
        if (in_spcblock)
            asar_throw_error(0, error_type_block,
                             error_id_feature_unavaliable_in_spcblock);
        if (numwords == 3) {
            if ((unsigned int)snespos & 0xFF000000)
            ...
            if (!stricmp(par, "JSL") || !stricmp(par, "JML")) {
                ...
                if (pass == 2) {
                    ...
                    extern AddressToLineMapping addressToLineMapping;
                    addressToLineMapping.includeMapping(get_current_file_name(),
                                                        get_current_line() + 1,
                                                        addrToLinePos);
                }
                // freespaceorglen[targetid]=read2(ratsloc-4)+1;
        ...
        if (!pushpcnum)
            asar_throw_error(0, error_type_block, error_id_pullpc_without_pushpc);
        pushpcnum--;
        freespaceend();
        if (arch != pushpc[pushpcnum].arch)
            asar_throw_error(0, error_type_block, error_id_pullpc_different_arch);
        snespos = pushpc[pushpcnum].snespos;
        startpos = pushpc[pushpcnum].snesstart;
        realsnespos = pushpc[pushpcnum].snesposreal;
        realstartpos = pushpc[pushpcnum].snesstartreal;
        ...
        if (!basestacknum)
            asar_throw_error(0, error_type_block, error_id_pullbase_without_pushbase);
        basestacknum--;
        ...
        if (in_spcblock)
            asar_throw_error(0, error_type_block,
                             error_id_feature_unavaliable_in_spcblock);
        if (!pushnsnum)
            asar_throw_error(0, error_type_block, error_id_pullns_without_pushns);
        pushnsnum--;
        ...
        if ((unsigned int)snespos & 0xFF000000)
            asar_throw_error(0, error_type_block, error_id_warnpc_in_freespace);
        if ((unsigned int)maxpos & 0xFF000000)
            asar_throw_error(0, error_type_block, error_id_warnpc_broken_param);
        if ((unsigned int)snespos > maxpos)
            asar_throw_error(0, error_type_block, error_id_warnpc_failed,
                             hex((unsigned int)snespos).data(),
                             hex((unsigned int)maxpos, 6).data());
    }
#ifdef SANDBOX
        ...
        if (!readfile(name, current_file, &data, &len))
            asar_throw_error(0, error_type_block,
                             vfile_error_to_error_id(asar_get_last_io_error()),
                             name.data());
        autoptr<char*> datacopy = data;
        if (!end) end = len;
        if (start < 0)
            asar_throw_error(0, error_type_block, error_id_file_offset_out_of_bounds,
                             dec(start).data(), name.data());
        if (end < start || end > len || end < 0)
            asar_throw_error(0, error_type_block, error_id_file_offset_out_of_bounds,
                             dec(end).data(), name.data());
        if (numwords == 4) {
            if (!confirmname(word[3])) {
                ...
                if (offset + end - start > 0xFFFFFF)
                    asar_throw_error(0, error_type_block, error_id_16mb_rom_limit);
                ...
                if (pass == 0) {
                    ...
                    if (pos < 0)
                        asar_throw_error(0, error_type_block, error_id_no_freespace,
                                         dec(end - start).data());
                    int foundfreespaceid = getfreespaceid();
                    freespacepos[foundfreespaceid] = pctosnes(pos) |
                                                     (/*fastrom?0x800000:*/ 0x000000) |
                                                     (foundfreespaceid << 24);
                    setlabel(word[3], freespacepos[foundfreespaceid]);
                    writeromdata_bytes(pos, 0xFF, end - start);
                }
                if (pass == 1) {
                    getfreespaceid();  // nothing to do here, but we want to tick the
                                       // counter
                }
                if (pass == 2) {
                    int foundfreespaceid = getfreespaceid();
                    if (freespaceleak[foundfreespaceid])
                        asar_throw_warning(2, warning_id_freespace_leaked);
                    writeromdata(snestopc(freespacepos[foundfreespaceid] & 0xFFFFFF),
        ...
        if (numwords != 2 && numwords != 3 && numwords != 5)
            asar_throw_error(0, error_type_block, error_id_unknown_command);
        if (numwords > 2 && stricmp(word[1], "align"))
            asar_throw_error(0, error_type_block, error_id_unknown_command);
        if (numwords == 5 && stricmp(word[3], "offset"))
            asar_throw_error(0, error_type_block, error_id_unknown_command);
        int amount;
        if (numwords > 2) {
            ...
            if (alignment > 0x800000)
                asar_throw_error(0, error_type_block, error_id_alignment_too_big);
            if (alignment < 1)
                asar_throw_error(0, error_type_block, error_id_alignment_too_small);
            if (alignment & (alignment - 1))
                asar_throw_error(0, error_type_block, error_id_invalid_alignment);
            // i just guessed this formula but it seems to work
            amount = (alignment - ((snespos - offset) & (alignment - 1))) &
                     (alignment - 1);
        } else {
        ...
        if (tablestack.count <= 0)
            asar_throw_error(0, error_type_block, error_id_pulltable_without_table);
        thetable = tablestack[tablestack.count - 1];
        tablestack.remove(tablestack.count - 1);
    } else if (is3("function")) {
        if (stricmp(word[2], "="))
            asar_throw_error(0, error_type_block, error_id_broken_function_declaration);
        if (!confirmqpar(word[1]))
            asar_throw_error(0, error_type_block, error_id_broken_function_declaration);
        string line = word[1];
        line.qnormalize();
        ...
        *startpar = 0;
        startpar++;
        if (!confirmname(line))
            asar_throw_error(0, error_type_block, error_id_invalid_function_name);
        char* endpar = strqchr(startpar, ')');
        // confirmqpar requires that all parentheses are matched, and a starting one
        // exists, therefore it is harmless to not check for nulls
        if (endpar[1])
            asar_throw_error(0, error_type_block, error_id_broken_function_declaration);
        *endpar = 0;
        ...
        if (0)
            ;
        else if (!stricmp(par, "bytes"))
            bytes = 0;
        else if (!stricmp(par, "freespaceuse"))
            freespaceuse = 0;
        else
            asar_throw_error(2, error_type_block, error_id_unknown_variable);
        ...
        if ((unsigned int)realsnespos & 0xFF000000)
            asar_throw_error(0, error_type_block, error_id_pad_in_freespace);
        int num = (int)getnum(par);
        if ((unsigned int)num & 0xFF000000)
            asar_throw_error(0, error_type_block,
                             error_id_snes_address_doesnt_map_to_rom,
                             hex((unsigned int)num, 6).data());
        if (num > realsnespos) {
    ...
    } else if (is1("fillbyte") || is1("fillword") || is1("filllong") ||
               is1("filldword")) {
        int len = 0;
        ...
        if (in_spcblock)
            asar_throw_error(0, error_type_block,
                             error_id_feature_unavaliable_in_spcblock);
        if (!stricmp(par, "65816")) {
            arch = arch_65816;
            return;
        }
        if (!stricmp(par, "spc700")) {
            arch = arch_spc700;
            return;
        }
        if (!stricmp(par, "superfx")) {
            arch = arch_superfx;
            return;
        }
    } else if (is0("{") || is0("}")) {
...
bool assemblemapper(char** word, int numwords) {
    ...
    if (0)
        ...
        if (!force_checksum_fix)
            checksum_fix_enabled = false;  // we don't know where the header is, so
                                           // don't set the checksum
        ...
        if (par) {
            ...
            if (!is_digit(par[0]) || par[1] != ',' || !is_digit(par[2]) ||
                par[3] != ',' || !is_digit(par[4]) || par[5] != ',' ||
                !is_digit(par[6]) || par[7])
                asar_throw_error(0, error_type_block, error_id_invalid_mapper);
            int len;
    ...
    if (in_spcblock)
        asar_throw_error(0, error_type_block, error_id_feature_unavaliable_in_spcblock);
    if (!mapper_set) {
        mapper_set = true;
