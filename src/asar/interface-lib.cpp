...
// note: somewhat fragile, assumes that every patchparams struct inherits from exactly
// the previous one
/* $EXPORTSTRUCT_PP$
 */
...
void print(const char* str) { prints[numprint++] = duplicate_string(str); }

static void fillerror(errordata& myerr, const char* errname, const char* type,
                      const char* str, bool show_block) {
    const char* current_filename = get_current_file_name();
    if (current_filename)
        myerr.filename = duplicate_string(current_filename);
    else
        myerr.filename = duplicate_string("");
    myerr.line = get_current_line();
    const char* current_block = get_current_block();
    if (current_block)
        myerr.block = duplicate_string(current_block);
    else
        myerr.block = duplicate_string("");
    myerr.rawerrdata = duplicate_string(str);
    string location;
    string details;
    get_current_line_details(&location, &details);
    myerr.fullerrdata =
            duplicate_string(location + ": " + type + str + details + get_callstack());
    ...
    myerr.callstack =
            static_cast<stackentry*>(malloc(sizeof(stackentry) * myerr.callstacksize));
...
void error_interface(int errid, int whichpass, const char* e_) {
    ...
    if (ismath)
        ...
        fillerror(errors[numerror++], get_error_name((asar_error_id)errid),
                  STR "error: (" + get_error_name((asar_error_id)errid) + "): ", e_,
                  show_block);
    } else {
...
void warn(int errid, const char* str) {
    ...
    fillerror(warnings[numwarn++], get_warning_name((asar_warning_id)errid),
              STR "warning: (" + get_warning_name((asar_warning_id)errid) + "): ", str,
              show_block);
}
...
static void asar_patch_begin(char* romdata_, int buflen, int* romlen_) {
    if (buflen != maxromsize) {
        ...
        memcpy(const_cast<unsigned char*>(romdata_r) /*we just allocated this, it's safe
                                                        to violate its const*/
               ,
               romdata_, (size_t)*romlen_);
    } else
        romdata_r = (unsigned char*)romdata_;
...
static void asar_patch_main(const char* patchloc) {
    if (!path_is_absolute(patchloc))
        asar_throw_warning(pass, warning_id_relative_path_used, "patch file");

    try {
        for (pass = 0; pass < 3; pass++) {
            initstuff();
            assemblefile(patchloc);
            // RPG Hacker: Necessary, because finishpass() can throws warning and
            // errors.
            callstack_push cs_push(callstack_entry_type::FILE,
                                   filesystem->create_absolute_path(nullptr, patchloc));
            finishpass();
        }
    } catch (errfatal&) {
    }
...
static bool asar_patch_end(char* romdata_, int buflen, int* romlen_) {
    ...
    if (buflen < romlen)
        asar_throw_error(pass, error_type_null, error_id_buffer_too_small);
    if (errored) {
        if (numerror == 0)
...
EXPORT void asar_close() { resetdllstuff(); }
 ...
 */
EXPORT int asar_maxromsize() { return maxromsize; }
 ...
 */
EXPORT enum mapper_t asar_getmapper() { return mapper; }
