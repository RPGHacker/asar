...
// randomdude999: remember to also update the .rc files (in res/windows/) when changing
// this. Couldn't find a way to automate this without shoving the version somewhere in
// the CMake files
...
int get_version_int() { return asarver_maj * 10000 + asarver_min * 100 + asarver_bug; }
...
bool setmapper() {
    ...
    for (size_t mapid = 0; mapid < sizeof(maps) / sizeof(maps[0]); mapid++) {
        ...
        for (int i = 0; i < 21; i++) {
            ...
            if (foundnull && c)
                score -= 4;  // according to some documents, NUL terminated names are
        ...
        if (highbits > 0 && highbits <= 14)
            score -= 21;  // high bits set on some, but not all, bytes = unlikely to be
                          // a ROM
        if ((romdata[snestopc(0x00FFDE)] ^ romdata[snestopc(0x00FFDC)]) != 0xFF ||
            (romdata[snestopc(0x00FFDF)] ^ romdata[snestopc(0x00FFDD)]) != 0xFF)
            score = -99999;  // checksum doesn't match up to 0xFFFF? Not a ROM.
    ...
    if (mapper == lorom) {
        if (mapperbyte == 0x23 &&
            (romtypebyte == 0x32 || romtypebyte == 0x34 || romtypebyte == 0x35))
            mapper = sa1rom;
    }
    return (maxscore >= 0);
}
...
string generate_call_details_string(const char* current_block, const char* current_call,
...
string format_stack_line(const printable_callstack_entry& entry,
    ...
    return indent + generate_filename_and_line(entry.prettypath, entry.lineno) +
           entry.details;
}

void push_stack_line(autoarray<printable_callstack_entry>* out,
                     const char* current_file, const char* current_block,
                     const char* current_call, int current_line_no, int indentation,
                     bool add_lines) {
    ...
    new_entry.details =
            strip_whitespace(generate_call_details_string(current_block, current_call,
                                                          indentation, add_lines)
                                     .raw());
    ...
    for (int i = callstack.count - 1; i >= 0; --i) {
        switch (callstack[i].type) {
            case callstack_entry_type::FILE:
                ...
                *location = generate_filename_and_line(
                        get_pretty_filename(current_file), current_line_no);
                *details = generate_call_details_string(current_block, current_call, 4,
                                                        false);
                return;
        ...
        switch (callstack[i].type) {
            case callstack_entry_type::FILE:
                if (current_file != nullptr) {
                    push_stack_line(out, current_file, current_block, current_call,
                                    current_line_no, indentation, add_lines);
                }
                current_file = callstack[i].content;
...
string get_simple_callstack() {
    ...
    if (current_call != nullptr && current_file != nullptr) {
        e += STR "\n    called from: " +
             generate_filename_and_line(get_pretty_filename(current_file),
                                        current_line_no) +
             ": [%" + current_call + "]";
    }
    return e;
...
static int getlenforlabel(int insnespos, int thislabel, bool exists) {
    ...
    if (!exists) {
        ...
        freespaced = true;
        return 2;
    } else if ((optimize_dp == optimize_dp_flag::RAM) && bank == 0x7E &&
               (word - dp_base < 0x100)) {
        return 1;
    } else if (optimize_dp == optimize_dp_flag::ALWAYS &&
               (bank == 0x7E || !(bank & 0x40)) && (word - dp_base < 0x100)) {
        return 1;
    } else if (optimize_address == optimize_address_flag::RAM && bank == 0x7E &&
               word < 0x2000) {
        return 2;
    } else if (optimize_address == optimize_address_flag::MIRRORS &&
               (bank == relaxed_bank || (!(bank & 0x40) && !(relaxed_bank & 0x40))) &&
               word < 0x2000) {
        return 2;
    } else if (optimize_address == optimize_address_flag::MIRRORS && !(bank & 0x40) &&
               !(relaxed_bank & 0x40) && word < 0x8000) {
        return 2;
    } else if (optimizeforbank >= 0) {
        if ((unsigned int)thislabel & 0xFF000000)
            return 3;
        else if (bank == (unsigned int)optimizeforbank)
            return 2;
        else
            return 3;
    } else if ((unsigned int)(thislabel | insnespos) & 0xFF000000) {
        if ((unsigned int)(thislabel ^ insnespos) & 0xFF000000)
            return 3;
        else
            return 2;
    } else if ((thislabel ^ insnespos) & 0xFF0000) {
        return 3;
    } else {
        return 2;
    }
...
void resolvedefines(string& out, const char* start) {
    ...
    while (*here) {
        if (here[0] == '\\' && here[1] == '\\') {
            ...
            bool first = (here == start ||
                          (here >= start + 4 && here[-1] == ' ' && here[-2] == ':' &&
                           here[-3] == ' '));  // check if it's the start of a block
            string defname;
            here++;
            ...
            if (depth != in_macro_def) {
                ...
                if (depth > in_macro_def)
                    asar_throw_error(0, error_type_line, error_id_invalid_depth_resolve,
                                     "define", "define", depth, in_macro_def);
                continue;
            }
            ...
            if (*here == '{') {
                ...
                if (!validatedefinename(defname))
                    asar_throw_error(0, error_type_line, error_id_invalid_define_name);
            } else {
            ...
            if (first) {
                ...
                // RPG Hacker: throw an error if we're trying to overwrite built-in
                // defines.
                ...
                switch (mode) {
                    ...
                    case append: {
                        if (!defines.exists(defname))
                            asar_throw_error(0, error_type_line,
                                             error_id_define_not_found, defname.data());
            ...
            notdefineset:
                if (!defname)
                    ...
                    if (!defines.exists(defname))
                        asar_throw_error(0, error_type_line, error_id_define_not_found,
                                         defname.data());
                    else {
...
void assemblefile(const char* filename) {
    incsrcdepth++;
    string absolutepath =
            filesystem->create_absolute_path(get_current_file_name(), filename);
    ...
    if (!filecontents.exists(absolutepath)) {
        ...
        if (!temp) {
            asar_throw_error(0, error_type_null,
        ...
        for (int i = 0; newfile.contents[i]; i++) {
            char* line = newfile.contents[i];
            if (!*line) continue;
            for (int j = 1; line[strlen(line) - 1] == ',' && newfile.contents[i + j];
                 j++) {
    ...
    while (in_macro_def > 0) {
        asar_throw_error(0, error_type_null, error_id_unclosed_macro,
                         macro_defs[in_macro_def - 1].data());
        ...
        if (stribegin(current_line, "macro ") && numif == numtrue) {
            ...
            if (!pass) {
                if (in_macro_def == 1)
                    ...
                    tomacro(current_line);
            }
        } else if (!stricmp(current_line, "endmacro") && numif == numtrue) {
            if (in_macro_def == 0)
                asar_throw_error(0, error_type_line, error_id_misplaced_endmacro);
            else {
                in_macro_def--;
                macro_defs.remove(in_macro_def);
                if (!pass) {
...
void parse_std_defines(const char* textfile) {
    ...
    if (content != nullptr) {
        ...
        while (*pos != 0) {
            ...
            if (*defval == 0) {
                // no value
                if (clidefines.exists(define_name))
                    asar_throw_error(pass, error_type_null,
                                     error_id_cmdl_define_override, "Std define",
                                     define_name.data());
            ...
            while (*defval == ' ' || *defval == '\t')
                defval++;  // skip whitespace in beginning
            if (*defval == '"') {
                defval++;  // skip opening quote
                while (*defval != '"' && *defval != 0) cleaned_defval += *defval++;
...
static void printsymbol_wla(const string& key, snes_label& label) {
    string line = hex((label.pos & 0xFF0000) >> 16, 2) + ":" +
                  hex(label.pos & 0xFFFF, 4) + " " + key + "\n";
    symbolfile += line;
}
...
string create_symbols_file(string format, uint32_t romCrc) {
    ...
    if (format == "wla") {
        ...
        symbolfile += "\n[source files]\n";
        const autoarray<AddressToLineMapping::FileInfo>& addrToLineFileList =
                addressToLineMapping.getFileList();
        ...
        const autoarray<AddressToLineMapping::AddrToLineInfo>& addrToLineInfo =
                addressToLineMapping.getAddrToLineInfo();
...
const char* get_current_block() {
    for (int i = callstack.count - 1; i >= 0; --i) {
        if (callstack[i].type == callstack_entry_type::LINE ||
            callstack[i].type == callstack_entry_type::BLOCK)
            return callstack[i].content.raw();
    }
    return nullptr;
}
