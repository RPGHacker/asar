...
static int getlenforlabel(int insnespos, int thislabel, bool exists)
{
	...
	printf("pass %d; label = $%02X:%04X; dp_base = %04X; opt_dp = %d; ", pass, bank, word, dp_base, optimize_dp);
	if (!exists)
	{
	...
	{
		printf("exit: freespace?\n");
		if ((unsigned int)(thislabel^insnespos)&0xFF000000) return 3;
		else return 2;
	}
	else if ((thislabel^insnespos)&0xFF0000){ printf("exit: no idea?\n"); return 3; }
	else { printf("exit: asar gave up?\n");  return 2;}
}
