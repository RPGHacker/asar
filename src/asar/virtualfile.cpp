...
public:
    ...
    virtual size_t get_size() { return m_length; }
...
public:
    physical_file() : m_file_handle(InvalidFileHandle) {}

    virtual ~physical_file() { close(); }

    virtual_file_error open(const string& path) {
        if (path != "") {
            // randomdude999: checking this before file regularity to improve error
            // messages
    ...
    virtual size_t get_size() { return (size_t)get_file_size(m_file_handle); }
...
void virtual_filesystem::initialize(const char** include_paths,
...
void virtual_filesystem::destroy() { m_include_paths.reset(); }


virtual_file_handle virtual_filesystem::open_file(const char* path,
    ...
    if (vft != vft_memory_file) {
        asar_throw_warning(0, warning_id_check_memory_file, path,
                           (int)warning_id_check_memory_file);
    }
...
size_t virtual_filesystem::read_file(virtual_file_handle file_handle, void* out_buffer,
...
virtual_filesystem::virtual_file_type virtual_filesystem::get_file_type_from_path(
        const char* path) {
...
void virtual_filesystem::add_memory_file(const char* name, const void* buffer,
                                         size_t length) {
...
string virtual_filesystem::create_absolute_path(const char* base, const char* target) {
    ...
    if (path_is_absolute(test_path)) {
        ...
        if (test_path != "" &&
            (m_memory_files.exists(test_path) || file_exists(test_path))) {
            path_to_use = test_path;
        } else {
