...
static void range(int min, int mid, int max) {
    if (mid < min || mid > max)
        asar_throw_error(0, error_type_block, error_id_superfx_invalid_register, min,
                         max);
}
...
static bool check_short_addr(int num) {
    if (num % 2 > 0 || num < 0 || num > 0x1FE) {
        asar_throw_error(0, error_type_block, error_id_superfx_invalid_short_address,
...
#define w(val) , write1((unsigned int)(val))
            int reg1;
            bool isreg1 = getreg(arg[0], &reg1, reg_r);
            int reg2;
            bool isreg2 = getreg(arg[1], &reg2, reg_r);
            if (isreg1) {
                if (isreg2) {
