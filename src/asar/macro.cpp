...
void startmacro(const char* line_) {
    thisone = nullptr;
    if (!confirmqpar(line_))
        asar_throw_error(0, error_type_block, error_id_broken_macro_declaration);
    string line = line_;
    line.qnormalize();
    ...
    *startpar = 0;
    startpar++;
    if (!confirmname(line))
        asar_throw_error(0, error_type_block, error_id_invalid_macro_name);
    defining_macro_name = line;
    char* endpar = startpar + strlen(startpar) - 1;
    // confirmqpar requires that all parentheses are matched, and a starting one exists,
    // therefore it is harmless to not check for nullptrs
    if (*endpar != ')')
        asar_throw_error(0, error_type_block, error_id_broken_macro_declaration);
    *endpar = 0;
    for (int i = 0; startpar[i]; i++) {
        char c = startpar[i];
        if (!is_ualnum(c) && c != ',' && c != '.' && c != ' ')
            asar_throw_error(0, error_type_block, error_id_broken_macro_declaration);
        if (c == ',' && is_digit(startpar[i + 1]))
            asar_throw_error(0, error_type_block, error_id_broken_macro_declaration);
    }
    if (*startpar == ',' || is_digit(*startpar) || strstr(startpar, ",,") ||
        endpar[-1] == ',')
        asar_throw_error(0, error_type_block, error_id_broken_macro_declaration);
    if (macros.exists(defining_macro_name))
        asar_throw_error(0, error_type_block, error_id_macro_redefined,
                         defining_macro_name.data());
    ...
    if (thisone->parent_macro != nullptr)
        thisone->parent_macro_num_varargs =
                current_macro_numargs - (current_macro->numargs - 1);
    for (int i = 0; thisone->arguments[i]; i++) {
        if (!strcmp(thisone->arguments[i], "...") && !thisone->arguments[i + 1])
            thisone->variadic = true;
        else if (!strcmp(thisone->arguments[i], "..."))
        ...
        else if (!confirmname(thisone->arguments[i]))
            asar_throw_error(0, error_type_block, error_id_invalid_macro_param_name);
        for (int j = i + 1; thisone->arguments[j]; j++) {
            if (!strcmp(thisone->arguments[i], thisone->arguments[j]))
                asar_throw_error(0, error_type_block, error_id_macro_param_redefined,
                                 thisone->arguments[i]);
        }
    }
    numlines = 0;
...
void endmacro(bool insert) {
    ...
    if (insert)
        macros.create(defining_macro_name) = thisone;
    else {
        freemacro(thisone);
        thisone = nullptr;
    }
...
void callmacro(const char* data) {
    ...
    macrodata* thismacro;
    if (!confirmqpar(data))
        asar_throw_error(0, error_type_block, error_id_broken_macro_usage);
    string line = data;
    line.qnormalize();
    char* startpar = (char*)strchr(line.data(), '(');
    ...
    *startpar = 0;
    ...
    if (!confirmname(line))
        asar_throw_error(0, error_type_block, error_id_broken_macro_usage);
    if (!macros.exists(line))
        asar_throw_error(0, error_type_block, error_id_macro_not_found, line.data());
    ...
    *endpar = 0;
    ...
    if (numargs != thismacro->numargs && !thismacro->variadic)
        asar_throw_error(1, error_type_block, error_id_macro_wrong_num_params);
    // RPG Hacker: -1, because the ... is also counted as an argument, yet we want it to
    // be entirely optional.
    if (numargs < thismacro->numargs - 1 && thismacro->variadic)
    ...
    for (int i = 0; i < numargs; ++i) {
        ...
        (*reinterpret_cast<autoptr<const char**>*>(&args))[i] =
                safedequote(strip_whitespace((char*)args[i]));
    }

    // RPG Hacker: -1 to take the ... into account, which is also being counted.
    if (thismacro->variadic)
        numvarargs = numargs - (thismacro->numargs - 1);
    else
        numvarargs = -1;
...

    {
        ...
        for (int i = 0; i < thismacro->numlines; i++) {
            ...
            do_line_logic(thismacro->lines[i], thismacro->fname,
                          thismacro->startline + i + 1);

            if (numif != prevnumif && whilestatus[numif].iswhile &&
                whilestatus[numif].cond)
...
string generate_macro_hint_string(const char* named_arg, const macrodata* thismacro,
                                  int desired_depth, int current_depth = 0) {
    ...
    if (thismacro != nullptr) {
        for (int j = 0; thismacro->arguments[j]; j++) {
            if (!strcmp(named_arg, thismacro->arguments[j])) {
                string ret = " Did you mean: '";
                ret += generate_macro_arg_string(thismacro->arguments[j],
                                                 desired_depth + current_depth);
                ret += "'?";
                return ret;
            }
        }
        return generate_macro_hint_string(named_arg, thismacro->parent_macro,
...
string generate_macro_hint_string(int var_arg, const macrodata* thismacro,
                                  int desired_depth, int current_depth = 0) {
    if (thismacro != nullptr) {
        if (thismacro->parent_macro_num_varargs > var_arg) {
            string ret = " Did you mean: '";
            ret += generate_macro_arg_string(var_arg,
                                             desired_depth + current_depth + 1);
            ret += "'?";
            return ret;
        }
        return generate_macro_hint_string(var_arg, thismacro->parent_macro,
                                          desired_depth, current_depth + 1);
    }
...
string replace_macro_args(const char* line) {
    ...
    for (const char* in = line; *in;) {
        if (*in == '<' && in[1] == '<' && in[2] != ':') {
            ...
            const char* end = in + 1;
            // RPG Hacker: Added checking for space here, because this code would
            // consider if a < b && a > c a macro arg expansion. In practice, this is
            // still a sloppy solution and is likely to fail in some edge case I can't
            // think of right now. Should parse this in a much more robust way at some
            // point...
            if (*end == ' ') {
                out += *(in++);
                continue;
            ...
            while (*end && *end != '>' && *end != '<' && *(end + 1) != ':')
                end++;  // allow for conditionals and <:
            if (*end != '>') {
                out += *(in++);
                continue;
            }
                ...
                if (depth > in_macro_def) {
                    if (in_macro_def > 0)
                        asar_throw_error(0, error_type_line,
                                         ...
                                         "macro parameter", "macro parameter", depth,
                                         in_macro_def - 1);
                    else
            ...
            if (!inmacro)
                asar_throw_error(0, error_type_block,
                                 error_id_macro_param_outside_macro);
            if (is_variadic_arg && !current_macro->variadic)
                asar_throw_error(0, error_type_block, error_id_macro_not_varadic,
                                 "<...[math]>");
            //*end=0;
            string param;
            ...
            if (!is_variadic_arg) {
                ...
                if (!found) {
                    asar_throw_error(
                            0, error_type_block, error_id_macro_param_not_found,
                            generate_macro_arg_string(in, depth).raw(),
                            generate_macro_hint_string(in, current_macro, depth).raw());
                }
            } else {
                snes_label ret;
                if (valid_named_param && !labelval(in, &ret, false))
                    asar_throw_error(0, error_type_block, error_id_invalid_vararg, in);
                ...
                if (arg_num < 0)
                    asar_throw_error(1, error_type_block, error_id_vararg_out_of_bounds,
                                     generate_macro_arg_string(arg_num, depth).raw(),
                                     "");
                if (arg_num > current_macro_numargs - current_macro->numargs)
                    asar_throw_error(
                            1, error_type_block, error_id_vararg_out_of_bounds,
                            generate_macro_arg_string(arg_num, depth).raw(),
                            generate_macro_hint_string(arg_num, current_macro, depth)
                                    .raw());
