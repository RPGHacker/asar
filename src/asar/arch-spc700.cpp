...
static bool matchandwrite(const char* str, const char* left, const char* right,
                          string& remainder) {
...
static bool bitmatch(const char* opnamein, string& opnameout, const char* str,
                     string& math, int& bit) {
...
static bool assinglebitwithc(const char* op, const char* math, int bits) {
    ...
    if (num >= 0x2000)
        asar_throw_error(2, error_type_block, error_id_spc700_addr_out_of_range,
                         hex(num, 4).data());
...
#undef op
        ...
        int opLen = 0;  // In case of .b or .w, this overwrites auto-detection of opcode
                        // length
        ...
        if (opLen > 2) {
            asar_throw_error(0, error_type_block, error_id_opcode_length_too_long);
        }
...
#define wv(hex1, hex2)                                           \
    do {                                                         \
        if ((opLen == 1) || (opLen == 0 && getlen(math) == 1)) { \
            write1((unsigned int)hex1);                          \
            write1(getnum(math));                                \
        } else {                                                 \
            write1((unsigned int)hex2);                          \
            write2(getnum(math));                                \
        }                                                        \
        return true;                                             \
    } while (0)
#define wr(hex)                                                                        \
    do {                                                                               \
        ...
        int pos = (len == 1) ? num : num - ((snespos & 0xFFFFFF) + 2);                 \
        write1((unsigned int)hex);                                                     \
...
#define op2(str, hex) \
    if (isop(str)) w2(hex)
#define opv(str, hex1, hex2) \
    if (isop(str)) wv(hex1, hex2)
#define opr(str, hex) \
    if (isop(str)) wr(hex)
#define match(left, right) if (ismatch(left, right))
            ...
            if (bitmatch(word[0], op, arg[0], math, bits)) {
                if (assinglebitwithc(op, math, bits))
                    return true;
                else if (!stricmp(op, "set"))
                    write1((unsigned int)(0x02 | (bits << 5)));
                else if (!stricmp(op, "clr"))
                    write1((unsigned int)(0x12 | (bits << 5)));
                else
                    return false;
            ...
            if (true) {
                ...
                if (isop("tcall")) {
                    unsigned int num = getnum(math);
                    if (num >= 16)
                        asar_throw_error(2, error_type_block, error_id_invalid_tcall);
                    write1(((num << 4) | 1));
                    return true;
                }
                opv("asl", 0x0B, 0x0C);
...
#define iscv(str1, left2, right2) \
    (!stricmp(arg[0], str1) && matchandwrite(arg[1], left2, right2, s2))
#define isvc(left1, right1, str2) \
    (matchandwrite(arg[0], left1, right1, s1) && !stricmp(arg[1], str2))
#define isvv(left1, right1, left2, right2)       \
    (matchandwrite(arg[0], left1, right1, s1) && \
     matchandwrite(arg[1], left2, right2, s2))
...
#define w1(opcode, math)                                             \
    do {                                                             \
        write1((unsigned int)opcode);                                \
        unsigned int val = getnum(math);                             \
        if ((((val & 0xFF00) && (val & 0x80000000) == 0) ||          \
             (((val & 0xFF00) != 0xFF00) && (val & 0x80000000))) &&  \
            opLen != 1)                                              \
            asar_throw_warning(0, warning_id_spc700_assuming_8_bit); \
        write1(val);                                                 \
        return true;                                                 \
    } while (0)
#define w2(opcode, math)              \
    do {                              \
        write1((unsigned int)opcode); \
        write2(getnum(math));         \
        return true;                  \
    } while (0)
#define wv(opcode1, opcode2, math)                               \
    do {                                                         \
        if ((opLen == 1) || (opLen == 0 && getlen(math) == 1)) { \
            write1((unsigned int)opcode1);                       \
            write1(getnum(math));                                \
        } else {                                                 \
            write1((unsigned int)opcode2);                       \
            write2(getnum(math));                                \
        }                                                        \
        return true;                                             \
    } while (0)
#define w11(opcode, math1, math2)     \
    do {                              \
        write1((unsigned int)opcode); \
        write1(getnum(math1));        \
        write1(getnum(math2));        \
...
#define wr(opcode, math)                                                               \
    do {                                                                               \
        int len = getlen(math);                                                        \
        int num = (int)getnum(math);                                                   \
        int pos = (len == 1) ? num : num - (snespos + 2);                              \
        if (pass && foundlabel && (pos < -128 || pos > 127))                           \
            asar_throw_error(2, error_type_block,                                      \
                             error_id_relative_branch_out_of_bounds, dec(pos).data()); \
        write1((unsigned int)opcode);                                                  \
        write1((unsigned int)pos);                                                     \
...
#define w1r(opcode, math1, math2)                                                      \
    do {                                                                               \
        int len = getlen(math2);                                                       \
        int num = (int)getnum(math2);                                                  \
        int pos = (len == 1) ? num : num - (snespos + 3);                              \
        if (pass && foundlabel && (pos < -128 || pos > 127))                           \
            asar_throw_error(2, error_type_block,                                      \
                             error_id_relative_branch_out_of_bounds, dec(pos).data()); \
        write1((unsigned int)opcode);                                                  \
...
#define isop(test) (!stricmp(op, test))
            ...
            if (bitmatch(word[0], op, arg[0], s1, bits)) {
                ...
                if (0)
                    ...
                    write1((unsigned int)(0x03 | (bits << 5)));
                else if (isop("bbc"))
                    write1((unsigned int)(0x13 | (bits << 5)));
                else
                    return false;
                unsigned int num = getnum(s1);
                if (num >= 0x100)
...
#undef isop
            if (is("mov")) {
                if (iscc("(x)+", "a"))
                    asar_throw_error(0, error_type_block, error_id_use_xplus);
                cc("(x+)", "a") w0(0xAF);
                cc("(x)", "a") w0(0xC6);
                if (iscc("a", "(x)+"))
                    asar_throw_error(0, error_type_block, error_id_use_xplus);
            ...
            if (is("or") || is("and") || is("eor") || is("cmp") || is("adc") ||
                is("sbc")) {
                int offset = 0;
