...
struct asar_warning_mapping {
    ...
    asar_warning_mapping(asar_warning_id inwarnid, const char* iname,
                         const char* inmessage, bool inenabled = true) {
        ++asar_num_warnings;
...
static asar_warning_mapping asar_warnings[] = {
        {WRN(relative_path_used),
         "Relative %s path passed to asar_patch_ex() - please use absolute paths only "
        ...
        {WRN(65816_xx_y_assume_16_bit),
         "%s $xx,y is not valid with 8-bit parameters, assuming 16-bit."},
        {WRN(spc700_assuming_8_bit),
         "This opcode does not exist with 16-bit parameters, assuming 8-bit."},

        {WRN(cross_platform_path),
         "This patch may not assemble cleanly on all platforms. Please use / instead."},

        {WRN(missing_org), "Missing org or freespace command."},
        {WRN(set_middle_byte),
         "It would be wise to set the 008000 bit of this address."},

        {WRN(unrecognized_special_command),
         "Unrecognized special command - your version of Asar might be outdated."},
        ...
        {WRN(xkas_deprecated),
         "xkas support is being deprecated and will be removed in a future version of "
         "Asar. Please use an older version of Asar (<=1.50) if you need it."},
        {WRN(xkas_eat_parentheses),
         "xkas compatibility warning: Unlike xkas, Asar does not eat parentheses after "
         "defines."},
        {WRN(xkas_label_access),
         "xkas compatibility warning: Label access is always 24bit in emulation mode, "
         "but may be 16bit in native mode."},
        {WRN(xkas_warnpc_relaxed),
         "xkas conversion warning : warnpc is relaxed one byte in Asar."},
        {WRN(xkas_style_conditional),
         "xkas-style conditional compilation detected. Please use the if command "
         "instead."},
        {WRN(xkas_patch),
         "If you want to assemble an xkas patch, add ;@xkas at the top or you may run "
         "into a couple of problems."},
        {WRN(xkas_incsrc_relative),
         "xkas compatibility warning: incsrc and incbin look for files relative to the "
         "patch in Asar, but xkas looks relative to the assembler."},
        {WRN(convert_to_asar),
         "Convert the patch to native Asar format instead of making an Asar-only xkas "
         "patch."},

        {WRN(fixed_deprecated),
         "the 'fixed' parameter on freespace/freecode/freedata is deprecated - please "
         "use 'static' instead."},

        {WRN(autoclear_deprecated),
         "'autoclear' is deprecated - please use 'autoclean' instead."},

        {WRN(check_memory_file),
         "Accessing file '%s' which is not in memory while W%d is enabled.", false},

        {WRN(if_not_condition_deprecated),
         "'if !condition' is deprecated - please use 'if not(condition)' instead."},
        ...
        {WRN(feature_deprecated),
         "DEPRECATION NOTIFICATION: Feature \"%s\" is deprecated and will be REMOVED "
         "in the future. Please update your code to conform to newer styles. Suggested "
         "work around: %s."},
...
static_assert(sizeof(asar_warnings) / sizeof(asar_warnings[0]) == warning_id_count,
              "asar_warnings and asar_warning_id are not in sync");

void asar_throw_warning(int whichpass, asar_warning_id warnid, ...) {
    if (pass == whichpass) {
        assert(warnid > warning_id_start && warnid < warning_id_end);

        const asar_warning_mapping& warning =
                asar_warnings[warnid - warning_id_start - 1];
