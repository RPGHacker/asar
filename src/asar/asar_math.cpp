// Don't try using this in your own project, it's got a lot of Asar-specific tweaks. Use
// mathlib.cpp instead.
#include "asar_math.h"
...
static cachedfile* opencachedfile(string fname, bool should_error) {
    ...
    if (cachedfilehandle != nullptr) {
        if (!cachedfilehandle->used) {
            ...
            if (cachedfilehandle->filehandle != INVALID_VIRTUAL_FILE_HANDLE) {
                ...
                cachedfilehandle->filesize =
                        filesystem->get_file_size(cachedfilehandle->filehandle);
                cachedfileindex++;
                // randomdude999: when we run out of cached files, just start
                // overwriting ones from the start
    ...
    if ((cachedfilehandle == nullptr ||
         cachedfilehandle->filehandle == INVALID_VIRTUAL_FILE_HANDLE) &&
        should_error) {
...
static int struct_size(const char* name) {
    if (pass && !structs.exists(name))
        asar_throw_error(2, error_type_block, error_id_struct_not_found, name);
    else if (!structs.exists(name))
        return 0;
...
static int data_size(const char* name) {
    ...
    if (!labels.exists(name))
        asar_throw_error(2, error_type_block, error_id_label_not_found, name);
    foundlabel = true;
    ...
    if (next_label == 0xFFFFFF)
        asar_throw_warning(2, warning_id_datasize_last_label, name);
    if (next_label - label > 0xFFFF)
        asar_throw_warning(2, warning_id_datasize_exceeds_size, name);
...
string get_string_argument() {
    ...
    return "";  // never actually called, but I don't feel like figuring out
                // __attribute__ ((noreturn)) on MSVC
}
...
double asar_bank(double a) { return (int)a >> 16; }


double asar_logical_nand(double a, double b) { return !(a && b); }


double asar_logical_nor(double a, double b) { return !(a || b); }


double asar_logical_xor(double a, double b) { return !!a ^ !!b; }

double asar_max(double a, double b) { return a > b ? a : b; }

double asar_min(double a, double b) { return a < b ? a : b; }
...
double asar_snestopc_wrapper() { return snestopc(get_double_argument()); }

double asar_pctosnes_wrapper() { return pctosnes(get_double_argument()); }

double asar_realbase_wrapper() {
    // need a better way to do this to make sure it is useful...
    // foundlabel=true;  //Going to consider this an implicit label because we don't
    // really have a better system
    return realsnespos;
}
...
double asar_read() {
    ...
    if (has_next_parameter()) {
        ...
        if (addr < 0)
            asar_throw_error(
                    2, error_type_block, error_id_snes_address_doesnt_map_to_rom,
                    (hex((unsigned int)target, 6) + " in read function").data());
        else if (addr + count > romlen_r)
            asar_throw_error(2, error_type_block, error_id_snes_address_out_of_bounds,
                             (hex(target, 6) + " in read function").data());
    }
...
double asar_canread() {
    ...
    if (addr < 0 || addr + length - 1 >= romlen_r)
        return 0;
    else
        return 1;
}
...
double asar_readfile() {
    ...
    if (!should_error) {
        ...
        if (offset < 0 || offset + count > fhandle->filesize)
            asar_throw_error(2, error_type_block, error_id_file_offset_out_of_bounds,
                             dec(offset).data(), name.data());
    }
...
// returns 0 if the file is OK, 1 if the file doesn't exist, 2 if it couldn't be opened
...
static double asar_isdefined() { return defines.exists(get_string_argument()); }
...
static double asar_structsize_wrapper() {
    ...
    if (symbol == "...") {
        if (!inmacro)
            asar_throw_error(2, error_type_block, error_id_vararg_sizeof_nomacro);
        if (numvarargs == -1)
            asar_throw_error(2, error_type_block, error_id_macro_not_varadic,
                             "sizeof(...)");
        return numvarargs;
    }
...
static double asar_char() {
    ...
    if (offset >= string1.length())
        asar_throw_error(2, error_type_block, error_id_oob, offset, string1.length());
    return string1[offset];
}

static double asar_strlen() { return get_string_argument().length(); }
...
static double asar_call_user_function() {
    ...
    for (int i = 0; i < user_function.numargs; i++) {
        if (!has_next) {
            asar_throw_error(2, error_type_block, error_id_expected_parameter,
    ...
    if (has_next) {
        asar_throw_error(2, error_type_block, error_id_unexpected_parameter,
                         current_user_function_name);
    }
    ...
    for (int i = 0; user_function.content[i]; i++) {
        ...
        for (int j = 0; user_function.arguments[j]; j++) {
            ...
            bool potential_arg =
                    stribegin(user_function.content + i, user_function.arguments[j]);
...
void createuserfunc(const char* name, const char* arguments, const char* content) {
    if (!confirmqpar(content))
        asar_throw_error(0, error_type_block, error_id_mismatched_parentheses);
    ...
    user_function.arguments =
            split(user_function.argbuf, ',', &(user_function.numargs));
    user_function.content = duplicate_string(content);
    for (int i = 0; user_function.arguments[i]; i++) {
        for (int j = 0; user_function.arguments[j]; j++) {
            if (i != j &&
                !stricmp(user_function.arguments[i], user_function.arguments[j])) {
                asar_throw_error(0, error_type_block, error_id_duplicate_param_name,
                                 user_function.arguments[i], name);
            }
        }
...
static double getnumcore() {
    if (*str == '$') {
        if (!is_xdigit(*++str))
            asar_throw_error(2, error_type_block, error_id_invalid_hex_value);
        uint64_t ret = 0;
    ...
    if (*str == '(') {
        ...
        double rval = eval(0);
        if (*str != ')')
            asar_throw_error(2, error_type_block, error_id_mismatched_parentheses);
        str++;
        return rval;
    }
    if (*str == '%') {
        if (str[1] != '0' && str[1] != '1')
            asar_throw_error(2, error_type_block, error_id_invalid_binary_value);
    ...
    if (*str == '\'') {
        ...
        if (*str != '\'')
            asar_throw_error(2, error_type_block, error_id_invalid_character);
        int64_t rval = thetable.get_val(orig_val);
        if (rval == -1) {
            // RPG Hacker: Should be fine to not check return value of
            // codepoint_to_utf8() here, because our error cases above already made sure
            // that orig_val contains valid data at this point.
            string u8_str;
            codepoint_to_utf8(&u8_str, orig_val);
            asar_throw_error(2, error_type_block, error_id_undefined_char,
                             u8_str.data());
        }
        str++;
        return rval;
...
#define prefix(sym, func)      \
    if (*str == sym) {         \
        str += 1;              \
        double val = getnum(); \
        return sanitize(func); \
    }
#define prefix2(sym, sym2, func)             \
    if (*str == sym && *(str + 1) == sym2) { \
        str += 2;                            \
        double val = getnum();               \
        return sanitize(func);               \
    }
...
// RPG Hacker: Same function as above, but doesn't truncate our number via int
// conversion
double getnumdouble(const char* instr) { return math(instr); }
    ...
    if (!strncmp(str, name, strlen(name))) { \
        ...
        oper("/", 5,
             right != 0.0 ? left / right
                          : oper_wrapped_throw(error_id_division_by_zero));
        oper("%", 5,
             right != 0.0 ? fmod((double)left, (double)right)
                          : oper_wrapped_throw(error_id_modulo_by_zero));
        oper("+", 4, left + right);
        oper("-", 4, left - right);
        oper("<<", 3,
             right >= 0.0 ? (int64_t)left << (uint64_t)right
                          : oper_wrapped_throw(error_id_negative_shift));
        oper(">>", 3,
             right >= 0.0 ? (int64_t)left >> (uint64_t)right
                          : oper_wrapped_throw(error_id_negative_shift));
...
double math(const char* s) {
    ...
    if (*str) {
        if (*str == ',')
            asar_throw_error(2, error_type_block, error_id_invalid_input);
        else
            asar_throw_error(2, error_type_block, error_id_mismatched_parentheses);
    }
    return rval;
...
void initmathcore() {
    functions.reset();
    builtin_functions.each(
            [](const char* key, double (*val)()) { functions[key] = val; });
